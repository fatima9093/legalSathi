...
class _EvidenceExtractorScreenState extends State<EvidenceExtractorScreen> {
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    _buildInfoBullet(
                      AppLocalizations.of(context)!.timestampsAndDates,
                    ),
                    _buildInfoBullet(
                      AppLocalizations.of(context)!.phoneNumbersAndEmails,
                    ),
                    _buildInfoBullet(
                      AppLocalizations.of(context)!.urlsAndSocialMedia,
                    ),
                    _buildInfoBullet(
                      AppLocalizations.of(context)!.threatClassificationLabel,
                    ),
                    _buildInfoBullet(
                      AppLocalizations.of(context)!.keyPhrasesAndEvidence,
                    ),
                  ],
                ),
              ),
  ...
  Future<void> _uploadScreenshots() async {
    ...
    try {
      ...
      if (result == null) {
        _showErrorSnackBar(l10n.noImagesSelected);
        return;
      }

      _processPickedFiles(result, 'image');
    } catch (e) {
      _showErrorSnackBar('${l10n.errorUploadingScreenshots}: ${e.toString()}');
  ...
  Future<void> _uploadTextLogs() async {
    ...
    try {
      ...
      if (result == null) {
        _showErrorSnackBar(l10n.noFilesSelectedTryAgain);
        return;
      }

      _processPickedFiles(result, 'text');
    } catch (e) {
      _showErrorSnackBar('${l10n.errorUploadingTextLogs}: ${e.toString()}');
  ...
  void _processPickedFiles(FilePickerResult result, String fileType) async {
    ...
    for (var file in result.files) {
      ...
      if (fileSize > (_maxFileSizeInMB * 1024 * 1024)) {
        _showErrorSnackBar(
          AppLocalizations.of(context)!.fileExceedsLimitMB(file.name),
        );
        continue;
      }
    ...
    if (validFiles.length + _uploadedFiles.length > 10) {
      _showErrorSnackBar(
        AppLocalizations.of(
          context,
        )!.maximumFilesAllowed(10 - _uploadedFiles.length),
      );
      return;
    }
    ...
    _showSuccessSnackBar(
      AppLocalizations.of(
        context,
      )!.filesAddedSuccessfullyCount(validFiles.length),
    );
  }
  ...
  void _extractEvidence() {
    if (_uploadedFiles.isEmpty) {
      _showErrorSnackBar(
        AppLocalizations.of(context)!.pleaseUploadAtLeastOneFile,
      );
      return;
    }
